...
Class Action {
	...
	function save_co_borrower(){
		extract($_POST);
		$data = " name_khmer = '$name_khmer' ";
		$data .= ", name_english = '$name_english' ";
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO co_borrowers set ".$data);
		}else{
			$save = $this->db->query("UPDATE co_borrowers set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_co_borrower(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM co_borrowers where id = ".$id);
		if($delete)
			return 1;
	}
	function save_loan(){
